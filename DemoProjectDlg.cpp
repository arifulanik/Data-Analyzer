...
CDemoProjectDlg* CDemoProjectDlg::basePointer = NULL; //Declaring NULL of the base pointer for data passing
...
BOOL CDemoProjectDlg::OnInitDialog()
{
	...
	m_pwndShow = &m_firstDialog; //Showing first dialog by default
...
void CDemoProjectDlg::buildTree()
{
	...
	fileHeader = TreeSoft->InsertItem("Used Variable", TVI_ROOT);
...
void CDemoProjectDlg::LoadCSVFile(CString& filePath, map<CString, vector<double>>& m_dataMap)
{
    ...
    while (getline(headerStream, headerItem, ','))
    {
		...
		m_dataMap[cstrA]; // Add the header item as a key in the map
		...
		m_List.InsertColumn(i, cstrA, LVCFMT_LEFT, 150);
		i++;
...
void CDemoProjectDlg::OnBnClickedLoadFile()
{
    ...
    if (dlg.DoModal() == IDOK)
    {
		...
		int sz = m_dataMap.size();
		CFirstDialog* obj = CFirstDialog::child;
		obj->CopyValues();

    }

	//For setting chart axes
	//int maxValInMap = INT_MIN, minValInMap = INT_MAX;
	/*for (auto key : m_dataMap)
	{
		for (int i = 0; i < key.second.size(); ++i)
		{
			if (key.second[i] >= maxValInMap)
			{
				maxValInMap = key.second[i];
			}
			if (key.second[i] <= minValInMap)
				minValInMap = key.second[i];
		}
	}*/
	//int X = maxValInMap;
	//int Y = minValInMap;
